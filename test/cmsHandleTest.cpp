...
// CHECK-FIXES: {{^}}  m_handle = iEvent.getHandle(m_token);{{$}}
...
// CHECK-MESSAGES: :[[@LINE-1]]:3: warning: direct call of function getByToken(EDGetTokenT<Foo>&, Handle<Foo>&) is deprecated and should be replaced here with handleVar4 = iEvent.getHandle(*ptoken). [cms-handle]
